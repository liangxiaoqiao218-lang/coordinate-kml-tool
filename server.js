...
function protectNaturalGoldJudgeOutput(text) {
  ...
  const artificialSmeltCue = /熔融流动|熔炼纹理|铸块|鑄塊|倒模|模具|金条|熔滴|人工加工|高度相似|整体光滑/i.test(source);

  if (!naturalGoldCue || artificialSmeltCue) {
    return source;
  }
  ...
  return source
    ...
    .replace(/确认是否为黄金/g, "检测成分和纯度")
    .replace(/确认是否为纯金/g, "检测成分和纯度")
...
【自然金块 / 砂金金块模式：优先保护自然金场景】
当图片中出现不规则金块、金粒、金豆状自然颗粒、河道采挖场景、电子秤称重、多块大小不一且形态不规则的黄色金属，或表面有自然磨圆、凹凸、孔洞、氧化、泥土残留时，优先归为“自然金块”或“砂金金块”。
这类图不要默认归为人工熔炼金属。必须使用类似表达：疑似自然金块 / 河道采挖自然金，外观具备自然金形态特征，但纯度仍需通过密度、XRF或火试金确认。
等级建议：清晰且主体明确可给C或B；可信度通常可给高。图片只能判断形态，不能确认纯度。
关键依据写：不规则形态、大小不一、自然磨圆、凹凸/孔洞、泥土或氧化残留、电子秤称重、河道采挖语境。
主要风险写：照片不能确认纯度；黄铜/镀层/低成色金属仍需检测排除。
下一步必须优先给：吊水密度测试、XRF光谱、火试金。

【人工熔炼金属模式：条件必须严格】
只有出现明显人工熔炼特征时才可归为人工熔炼金属，例如：表面有明显熔融流动痕迹、边缘圆滑像倒模、多块形态高度相似、有铸块/金条/熔滴/模具痕迹、表面整体光滑且人工加工感强。
...
【下一步】
给2条具体动作。自然金块/砂金金块优先写吊水密度测试、XRF光谱、火试金；只能写检测成分和纯度，不要写“确认是否为黄金”“确认是否为纯金”这类照片外观导向确认的表述。原矿石可写敲开看断面、加比例物拍照、补拍原地环境或河道上下游图。
