...
function protectNaturalGoldJudgeOutput(text) {
  ...
  const artificialCueText = source.replace(/无明显(?:人工)?熔炼[^。\n；;]*/g, "");
  const strongArtificialSmeltCue = explicitArtificialSmeltScene || (
  ...
  return source
    ...
    .replace(/确认(?:是否)?为(?:黄金|纯金)/g, "检测成分和纯度")
...
【自然金块 / 熔炼金属判断顺序】
第一步先判断是否有明显人工熔炼痕迹。如果有，不要归自然金块。
人工熔炼优先特征：形态接近圆饼、半球、滴状、块状凝固体；边缘整体圆滑，有液态金属凝固后的圆边；表面有熔融冷却后的皱褶、气孔、凹坑、流痕；底部或局部呈扁平面，像倒在平面上冷却形成；多个金块形态相似，像同一批熔炼出来；缺少天然砂金常见的冲刷磨圆、石英/黑砂/泥土附着、自然裂隙和不规则边界。
出现这些特征时，场景分类优先写“人工熔炼金属”或“熔炼金豆”，结论写：外观更接近熔炼后形成的金属块，不宜直接当自然金块判断。
典型熔炼金豆例子：手掌上两块黄色金属，形态像圆饼/半球，边缘整体圆滑，表面有蜂窝状凹坑、冷却皱褶或凝固面，即使背景在户外，也应归为人工熔炼金属 / 熔炼金豆。

第二步再判断是否符合自然金块。只有同时具备明显不规则形态、边缘不是整体圆滑凝固边、表面有自然磨蚀/冲刷/凹凸结构，可能有泥土/氧化物/黑砂/石英残留，并且处在河道采挖、砂金清洗、矿区现场语境中，才优先归为“自然金块”或“砂金金块”。
...
第三步如果无法区分自然金块和熔炼金属，输出“疑似金属块，需检测确认”，不要强判自然金块。
...
当图片更像圆滑熔炼块、熔炼金豆时，禁止输出“自然金块”“河道采挖自然金”。不要仅凭颜色判断为自然金。
...
【下一步】
给2条具体动作。自然金块/砂金金块优先写吊水密度测试、XRF光谱、火试金；只能写检测密度、成分和纯度，不要写“确认是否为黄金”“确认是否为纯金”“确认密度是否符合黄金标准”这类照片外观导向确认的表述。原矿石可写敲开看断面、加比例物拍照、补拍原地环境或河道上下游图。
