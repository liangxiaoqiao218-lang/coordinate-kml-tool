...
export function extractDmsSourceStructure(text) {
  ...
  const closeGroup = closeReason => {
    if (currentRows.length) {
      groups.push(Object.freeze({ name: currentName || null, rows: Object.freeze([...currentRows]) }));
      reason ||= closeReason;
    }
    currentRows = [];
    currentName = "";
  ...
  for (const sourceLine of sourceLines(text)) {
    ...
    if (isTableHeader(line)) {
      if (currentRows.length) closeGroup("repeated_table_header");
      continue;
    }
    ...
    if (currentRows.length && pendingBlankBoundary) {
      closeGroup("blank_line");
    } else if (currentRows.length && !currentName && Number.isInteger(rowNumber)
      && Number.isInteger(previousRowNumber) && rowNumber <= previousRowNumber) {
  ...
  const displayText = groups.map(group => [group.name, ...group.rows].filter(Boolean).join("\n")).join("\n\n");
  return Object.freeze({
...
export function hasExplicitDmsMultiRegionEvidence(text, dmsGroupedInfo = {}) {
  ...
  const namedIdentities = sourceStructure.groups
    ...
    .filter(Boolean);
  const strongNumberRestart = sourceStructure.reason === "number_restart"
  ...
  return Boolean(
    hasDmsGroupBoundaryContext(text)
    || namedIdentities.length >= 2
    || strongNumberRestart
  );
}
...
export function evaluateDmsGroupedRetryEligibility({
  ...
  const needsRepair = explicitEvidence && (!dmsGroupedInfo?.output
    || (Number(groupedRowCount) >= 4 && Number(lonLatOrderLines) >= Math.max(2, Math.ceil(Number(groupedRowCount) * 0.5))));
  if (!isImageInput || structure.rowCount === 0 || !explicitEvidence || !needsRepair) {
